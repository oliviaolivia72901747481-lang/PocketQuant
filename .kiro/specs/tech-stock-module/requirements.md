...
2. WHEN RSI(14) 在 55-80 之间 THEN THE Signal_Generator SHALL 满足动量条件
3. WHEN 当日成交量 > 5日均量 × 1.5 THEN THE Signal_Generator SHALL 满足量能条件
4. WHEN 营收或净利至少一个正增长 且 无大额解禁 THEN THE Signal_Generator SHALL 满足基本面条件
5. WHEN 趋势、动量、量能、基本面四个条件同时满足 THEN THE Signal_Generator SHALL 生成买入信号
6. IF 任一条件不满足 THEN THE Signal_Generator SHALL 不生成买入信号
...
1. WHEN 持仓亏损达到 -10% THEN THE Exit_Manager SHALL 生成清仓卖出信号
2. WHEN 持仓盈利 > 5% THEN THE Exit_Manager SHALL 将止损位移至成本价
3. WHEN 持仓盈利 > 15% THEN THE Exit_Manager SHALL 将止损位移至 MA5
4. THE Exit_Manager SHALL 确保含手续费的亏损控制在1650元以内

### Requirement 7: RSI分仓止盈

**User Story:** As a 科技股投资者, I want 系统根据RSI和持仓量提供分仓止盈建议, so that 我可以锁定部分利润。

#### Acceptance Criteria

1. WHEN 持仓 >= 200股 且 RSI > 85 THEN THE Exit_Manager SHALL 建议卖出一半仓位
2. WHEN 持仓 = 100股 且 RSI > 85 THEN THE Exit_Manager SHALL 建议止损紧贴 MA5
3. THE Exit_Manager SHALL 在界面上显示当前RSI值和止盈建议
...
3. WHEN 触发 -10% 止损 THEN THE Exit_Manager SHALL 生成"止损"信号（第二优先级）
4. WHEN RSI > 85 触发止盈 THEN THE Exit_Manager SHALL 生成"止盈"信号（第三优先级）
